...
function createCalendar(layout, firstDay, numbDays, monthNum, yearNum) {
    ...
    for (var m = 0; m < wordDay.length; m++) {
        calendarString += '<th><a href="' + baseUrl + "?date=" + wordDay[m].substring(0, 3) + '">' + wordDay[m].substring(0, 3) + '</a><\/th>';
    }
    calendarString += '<\/tr>';
        ...
        if (k < (firstDay + numbDays)) {
            ...
            for (var x = 1; x <= 7; x++) {
                ...
                if ((daycounter > numbDays) || (daycounter < 1)) {
                    ...
                    if (date_start == 'sunday') {
                        if ((x == 1) || (x == 7)) {
                            daycounter_s = '<span class=\"calendar-day-weekend pointer\" onclick=gotoDate("' + yearNum + "-" + monthNum + "-" + daycounter + '");>' + daycounter + '</span>';
                        } else {
//                            daycounter_s = daycounter;
                            daycounter_s = '<span class=\"pointer\" onclick=gotoDate("' + yearNum + "-" + monthNum + "-" + daycounter + '");>' + daycounter + '</span>';
                        }
                    } else {
                        if ((x == 6) || (x == 7)) {
...
function gotoDate(date) {
    this.location = baseUrl + "?date=" + date;
}
...
//                + (tiva_events[id].event_image ? '<div class="event-image">' + tiva_events[id].event_image + '</div>' : '')
//                + (tiva_events[id].description ? '<div class="event-intro">' + getShortText(tiva_events[id].description, 10) + '</div>' : '')
//                + (tiva_events[id].created_by ? '<div class="event-intro">' + getShortText(tiva_events[id].created_by, 10) + '</div>' : '')
//                + '</div>'
//                );
...
function showEventList(layout, max_events) {
    ...
    if (layout == 'full') {
        ...
        for (var i = 0; i < tiva_list_events.length; i++) {
            ...
            jQuery('.tiva-event-list-full').append(
...
//                    + '<div class="event-item-right pull-left">'
                    ...
                    + '<div>' + getShortText(tiva_list_events[i].created_by, 25) + '</div>'            
                    + (tiva_list_events[i].client ? '<div>' + t.client + ' - ' + getShortText(tiva_list_events[i].client, 25) + '</div>' : '')
                    + (tiva_list_events[i].phone ? '<div>' + t.phone_no + ' - ' + getShortText(tiva_list_events[i].phone, 25) + '</div>' : '')
                    + (tiva_list_events[i].confirmed == 1 ? '<i class="fa fa-check"></i>' : '')
                    + '</div>' 
                    + '</div>' 
                    + '</div>'
                    );
...
function showEventDetail(id, layout, day, month, year) {
    ...
    if(tiva_events[id].idbooking > 0 === false) {
        return;
    }
    jQuery('#modalEventView').modal('show');
    jQuery('#modalEventView input[name=idbooking]').val(tiva_events[id].idbooking);
    jQuery('#modalEventView textarea[name=remark]').val(tiva_events[id].remark);
    jQuery('#modalEventView #description').html(tiva_events[id].description);
    jQuery('#modalEventView input[name=book_date]').val(tiva_events[id].year + '-' + tiva_events[id].month + '-' + tiva_events[id].day);
    jQuery('#modalEventView select[name=idtimeslot]').val(tiva_events[id].idtimeslot);
    jQuery('#modalEventView #idbooking').html(tiva_events[id].idbooking);
    jQuery('#modalEventView #created_by').html(tiva_events[id].created_by);
    jQuery('#modalEventView #updated_by').html(tiva_events[id].updated_by);
    jQuery('#modalEventView #provider').html(tiva_events[id].provider);
    jQuery('#modalEventView #product').html(tiva_events[id].product);
    if (tiva_events[id].modalPremium > 0) {
        jQuery('#modalEventView #modalPremium').html(tiva_events[id].modalPremiumCurrency + ' ' + tiva_events[id].modalPremium);
    }
    if (tiva_events[id].sumInsured > 0) {
        jQuery('#modalEventView #sumInsured').html(tiva_events[id].sumInsuredCurrency + ' ' + tiva_events[id].sumInsured);
    }
    jQuery('#modalEventView #client').html(tiva_events[id].client);
    jQuery('#modalEventView #phone_no').html(tiva_events[id].phone_no);
    jQuery('#modalEventView #history').html(tiva_events[id].history);
    jQuery('#modalEventView #instructions').html(tiva_events[id].instructions);
    jQuery('#modalEventView #documents').html(tiva_events[id].documents);

    if (tiva_events[id].reschedule_accepted === 1) {
        jQuery('#modalEventView #reschedule_accepted').html("<i class='zmdi zmdi-check-circle text-success'></i>");
    } else {
        jQuery('#modalEventView #reschedule_accepted').html("<i class='zmdi zmdi-close-circle text-danger'></i>");
    }
    if (tiva_events[id].requested_delete === 1) {
        jQuery('#modalEventView #requested_delete').html("<i class='zmdi zmdi-check-circle text-success'></i>");
    } else {
        jQuery('#modalEventView #requested_delete').html("<i class='zmdi zmdi-close-circle text-danger'></i>");
    }

    if (tiva_events[id].confirm == 1) {
        jQuery('#modalEventView .eventBtn').removeClass('btn-warning');
        jQuery('#modalEventView .eventBtn').addClass('btn-danger');
        jQuery('#modalEventView .confirmBtnText').html(t.cancel);
    } else {
        jQuery('#modalEventView .eventBtn').removeClass('btn-danger');
        jQuery('#modalEventView .eventBtn').addClass('btn-warning');
        jQuery('#modalEventView .confirmBtnText').html(t.confirm);
    }
...
jQuery(document).ready(function () {
    ...
    if (jQuery('.tiva-events-calendar.full').length) {
        jQuery('.tiva-events-calendar.full').html('<div class="events-calendar-bar">'
                ...
                + '<div class="tiva-event-list-full tiva-event-list row"></div>'
    ...
    if (source == 'json') { // Get events from json file : /js/events.json
